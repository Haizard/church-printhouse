            ...
            <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) setEditingPost(null); }}>
              ...
              <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 py-4">
                  ...
                  <div className="space-y-2"><Label htmlFor="imageUrl">Image URL</Label><Input id="imageUrl" name="imageUrl" defaultValue={editingPost?.imageUrl} placeholder="https://..." /></div>
                  ...
                  <div className="space-y-2"><Label htmlFor="content">Content</Label><Textarea id="content" name="content" defaultValue={editingPost?.content} rows={10} required /></div>
                  <DialogFooter><Button type="submit" disabled={isSubmitting} className="w-full">{isSubmitting ? <Loader2 className="animate-spin h-4 w-4" /> : (editingPost ? "Update" : "Publish")}</Button></DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow><TableCell colSpan={5} className="text-center py-10 text-muted-foreground">No posts found. Start writing your first story!</TableCell></TableRow>
                ) : posts?.map((post) => (
